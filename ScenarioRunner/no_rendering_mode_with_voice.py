...
class MapImage(object):
    ...
    def get_big_map_surface(self):
        return self.big_map_surface
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def draw_arrow(surface, transform, color=COLOR_ALUMINIUM_2):
            print(f'--------draw arrow transform: {transform}----------')
...
class ModuleWorld(object):
    ...
    def render(self, display):
        ...
        if self.hero_actor is not None:
            ...
            hero_location = self.hero_actor.get_location()
            hero_location_tuple = (hero_location.x, hero_location.y)

            #print(f'HERO LOCATION: {hero_location}, type: {type(hero_location)}')
            #print(f' tup:{hero_location_tuple}')

            #int(math.ceil(4.0 * self.map_image.scale))

            pygame.draw.line(self.map_image.get_big_map_surface(), COLOR_CHAMELEON_0, self.map_image.world_to_pixel(carla.Location(x=130,y=55)), self.map_image.world_to_pixel(carla.Location(x=92.6, y=55)), width = 10)
            pygame.draw.line(self.map_image.get_big_map_surface(), COLOR_CHAMELEON_0, self.map_image.world_to_pixel(carla.Location(x=92.6,y=55)), self.map_image.world_to_pixel(carla.Location(x=92.6, y=0.7)), width = 10)
            pygame.draw.line(self.map_image.get_big_map_surface(), COLOR_CHAMELEON_0, self.map_image.world_to_pixel(carla.Location(x=92.6,y=0.7)), self.map_image.world_to_pixel(carla.Location(x=198, y=0.7)), width = 10)

            #pedestrian_location_tuple = (92.6, 30)
            blah = (130, 55)

            pedestrian_potential_x = [90.0, 95.0]
            pedestrian_potential_y = [25.0, 35.0]
            ...
            if hero_location.x == blah[0] and hero_location.y == blah[1]:
                print("-----------------Run Voice Command: this worked--------------------")
                self.run_voice_command("Romany Romany Romany Romany! Pedestrian Detected! Halt! Romany Romany Romany Romany Romany Romany Romany")
            """

            """
            starting location: 130, 55
            first intersection: 100, 55
            pedestrian: 91, 40
            second intersection : 95, 5
            end: 198, 2
            """
...
def game_loop(args):
    try:
        ...
        text_surface = font.render('Running Differential Analysis using Wizard of Oz Experiment...' ,True, COLOR_WHITE)
